...
!      real,PARAMETER::mitiv=30!2.52E+00)                              ariyama1
	  ...
	  !振幅？
!	  PARAMETER (AMP=2.0d-2)                                                  !27.9GHz_基本
	  !PARAMETER (AMP=1.0d-1)                                                  !uemura                     !27.9GHz
	  !PARAMETER (AMP=2.0d-1)                                                   !ariyama     kihon
!	  PARAMETER (AMP=4.0d-2)                                                     !27.9GHz_振幅大
	  !PARAMETER (AMP=5.0d-2)                                                   !ariyama                         !振幅
	  PARAMETER (AMP=1.0d-2)                                                      !ariyama
!	  PARAMETER (AMP=5.0d-3)                                                      !27.9Ghz_振幅小
!	  PARAMETER (AMP=1.0d-3)                                                         !27.9Ghz_最小
	  ...
	  !PARAMETER (ROADRAMUDA=HIKARIRAMUDA)                              !uemura初期
	  !PARAMETER (ROADRAMUDA=HIKARIRAMUDA/3.0)
	  !PARAMETER (ROADRAMUDA=HIKARIRAMUDA/4)
	  PARAMETER (ROADRAMUDA=HIKARIRAMUDA*2)                           !周期大           !8.24距離測定、9.22      0.043m
...
!-----------------------------------------------------------------------
	  ...
	  DO I=1,NX
		!RFxn(I)=0
		!if(I.gt.1200.and.I.le.1400)then    !散乱体
		!RFxn(I)=AMP*sin(2.*PI*1./roadramuda*(I-1)*DX)                      !uemura
		!if(mod(I,10)==0)then
		!	RFxn(I)=0
		...
		!if(mod(I,10)==0)then
		!	RFxn(I)=0
		!else if(mod(I,10)==1)then
		!	RFxn(I)=0
		!else if(mod(I,10)==2)then
		!	RFxn(I)=2.0d-2
		!else if(mod(I,10)==3)then
		!	RFxn(I)=2.0d-2
		!else if(mod(I,10)==4)then
		!	RFxn(I)=0
		!else if(mod(I,10)==5)then
		!	RFxn(I)=0
		!else if(mod(I,10)==6)then
		!	RFxn(I)=2.0d-2
		!else if(mod(I,10)==7)then
		!	RFxn(I)=2.0d-2
		!else if(mod(I,10)==8)then
		!	RFxn(I)=0
		!else if(mod(I,10)==9)then
		!	RFxn(I)=0
		
		!RFxn(I)=AMP*sin(2.*PI*1./roadramuda*(I-1)*DX)                      !uemura
		
			
		!if(I.gt.500.and.I.le.550)then    !散乱体
		!if(I.gt.700.and.I.le.750)then                                      !右から
		
		
		
		
!		if(I.gt.3600.and.I.le.3800)then                                                 
!        if(I.gt.5800.and.I.le.6000)then                                                              !kyorihako採用
!		    RFxn(I)=0.06                                                                             !kyorihako_saiyou
		
!		if(I.gt.1400.and.I.le.1500)then                                                              !kyorihako採用        !8.4
!		    RFxn(I)=0.04                                                                       !kyorihako_saiyou           !8.4
...
!		else                                                         
			...
			!RFxn(I)=AMP*sin(2.*PI*1./roadramuda*(I-1)*DX)
!		    RFxn(I)=0
		
!		end if                                                                          !ここまで
		
         RFxn(I)=AMP*sin(2.*PI*1./roadramuda*(I-1)*DX)                              !普段はこれのみ
